...
export function TaskPanel(): React.JSX.Element {
  const { tasks, activeTaskId, setActiveTask, runTask, removeTask } = useTaskStore()
  ...
  const [availableTasks, setAvailableTasks] = useState<{name: string, command: string, args: string[]}[]>([])
  ...
  useEffect(() => {
    if (activeSession?.workspacePath) {
      ...
      const packageJsonPath = `${activeSession.workspacePath}/package.json`.replace(/\\/g, '/')
      window.electronAPI.fs.readFile(packageJsonPath)
        .then(content => {
          try {
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {activeTab === 'terminal' && (
          <div style={{ display: 'flex', height: '100%' }}>
            ...
            {/* Terminal Output */}
            <div style={{ flex: 1, padding: '12px', background: '#0D1117', color: '#E6EDF3', fontFamily: 'var(--font-mono)', fontSize: '13px', overflowY: 'auto' }}>
              {activeTask ? (
                <>
              ...
              <div key={`${t.id}-${i}`} style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '12px', border: '1px solid var(--border)', borderRadius: '6px', marginBottom: '8px', background: 'var(--bg-secondary)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <Button size="sm" variant="outline" onClick={() => {
                    // Send message to chat store to ask Lumiq to fix
                    const message = `Please fix this problem in ${p.file} at line ${p.line}:\n\n${p.message}`
                    // We can emit a custom event or just use the chat store directly here but we don't have chat store imported.
                    // For now, just copy to clipboard
                    navigator.clipboard.writeText(message)
                    alert('Copied problem to clipboard. Paste it in the chat to ask Lumiq to fix.')
                  }}>
                    Copy for AI
                  </Button>
                </div>
